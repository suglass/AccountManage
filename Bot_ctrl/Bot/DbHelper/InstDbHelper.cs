...
namespace DbHelper
{
    public class InstDbHelper : DbBaseHelper
    {
        ...
        public List<List<InstAccount>> GetAllAccounts()
        {
            List<List<InstAccount>> lst_accs = new List<List<InstAccount>>();
            try
            {
                List<string> lstr_group_names = GetGroupNames();

                foreach (string group_name in lstr_group_names)
                {
                    List<InstAccount> lst_accounts = GetAccountsByGroup(group_name);
        ...
        public List<string> GetGroupNames()
        {
            ...
            DataTable dt = select("SELECT group_name FROM `{0}` WHERE user_id = '{1}';", ConstEnv.inst_db_group_table_name, Program.g_user_id);
            foreach (DataRow row in dt.Rows)
                lstr_group_names.Add(row["group_name"].ToString());
            return lstr_group_names;
        }

        public List<InstAccount> GetAccountsByGroup(string group_name)
        {
            ...
            DataTable dt = select("SELECT accounts.id, accounts.username, accounts.userpass, accounts.emailaddr," +
                ...
                ConstEnv.inst_db_user_agent_table_name, ConstEnv.inst_db_account_table_name, group_name, Program.g_user_id);
        ...
        public bool InsertAccount(string file_line, string group_name)
        {
            ...
            string mail_server = string.Empty;
            string mail = string.Empty;
            string mailpass = string.Empty;

            int mail_server_id = -1;
            ...
            if (vals.Length == 6)
            {
                ...
                mail_server_id = GetMailServerId(mail_server);
            }
            ...
            {
                if (dt.Rows[0]["emailaddr"].ToString() == null || dt.Rows[0]["emailaddr"].ToString() == string.Empty)
                {
                    DataTable dt2 = select("SELECT * FROM {0} WHERE username = '{1}';", ConstEnv.inst_db_account_table_name, user);
        ...
        public void DeleteGroup(string group_name)
        {
            ...
            delete("DELETE FROM `{0}` WHERE group_id = '{1}' AND user_id = '{2}';", ConstEnv.inst_db_account_table_name, group_id, Program.g_user_id);
        ...
        public void UpdateAccount(int _acc_id, string _username, string _userpass, string _emailaddr, string _emailpass, string _proxy, string _user_agent)
        {
        ...
        public void UpdateUser(string _strUser, string _strPass)
        {
            update("UPDATE `{0}` SET user = '{1}', password = '{2}' WHERE id = '{3}';",
                ConstEnv.inst_db_user_table_name, _strUser, _strPass, Program.g_user_id);
        }
